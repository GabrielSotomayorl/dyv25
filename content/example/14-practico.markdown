---
title: "Análisis Cuantitativo Bivariado: Dispersión y Correlación"
linktitle: "14: Dispersión y Correlación"
date: "2025-11-24"
menu:
  example:
    parent: Ejemplos
...
### 1.3 Limpieza y Selección de Variables
...
2.  **Línea Azul (Loess):** Nos cuenta una historia más rica. El trabajo doméstico aumenta rápidamente entre los 15 y los 30 años (independencia, formación de hogares), se mantiene alto y estable durante la adultez, y comienza a decaer levemente en la vejez avanzada.
...
El resultado es **0.204**. ¿Cómo leemos esto?
...
En las ciencias sociales rara vez analizamos solo dos variables aisladas. Necesitamos ver el sistema completo. La función base `cor()` devuelve una matriz difícil de leer. El paquete `corrr` soluciona esto devolviendo un `tibble` (dataframe) fácil de manipular y visualizar.
...
1.  Crea una nueva variable `trabajo_total_nr` (ya disponible si ejecutaste el paso 1.3).
2.  **Filtra** la base para analizar solo a personas **ocupadas** (`t_to_dt > 0`), ya que queremos ver el efecto del empleo.
3.  Genera un gráfico de dispersión facetado por sexo:
    *   Eje X: `t_to_dt` (Horas remuneradas)
    *   Eje Y: `trabajo_total_nr` (Horas no remuneradas)
    *   Usa `facet_wrap(~ sexo_factor)`
    *   Añade `geom_smooth(method = "lm")`.
4.  **Calcula la correlación** (`cor`) agrupada por `sexo_factor`.
