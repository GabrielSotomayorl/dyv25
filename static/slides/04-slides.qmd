...
:::
::: {.column width="50%"}
![](https://i.imgur.com/8aB3t8g.png){fig-align="center"}
*La lógica es simple: no necesitamos comernos toda la sopa para saber si está salada. Una cucharada bien revuelta (una buena muestra) es suficiente.*
:::
::::::
...
Aquí es donde la teoría se encuentra con la cruda realidad.
...
**El problema fundamental:** El marco muestral casi nunca es una representación perfecta de la población objetivo. Esta brecha es una de las principales fuentes de error.
...
![](https://i.imgur.com/zWz2sC3.png){fig-align="center"}
*(Nota: Aquí iría un diagrama simple de una población de la que se extraen aleatoriamente individuos para formar la muestra).*
...
2.  **Ineficiencia y Costo:** Incluso si tuviéramos la lista, una muestra MAS para una encuesta cara a cara sería logísticamente una pesadilla. Los seleccionados estarían dispersos por todo el país, desde Arica hasta Punta Arenas, haciendo el trabajo de campo inviable.
...
-   **Lógica:** "Divide y vencerás". En lugar de muestrear de la población total, primero la dividimos en subgrupos homogéneos y mutuamente excluyentes llamados **estratos**. Luego, se realiza un muestreo aleatorio *dentro* de cada estrato.
...
-   **Ejemplo (CASEN):** Los estratos se conforman por la combinación de **Comuna-Área (Urbana/Rural)-Nivel Socioeconómico**.

![](https://i.imgur.com/kS4tqO7.png){fig-align="center"}
*(Nota: Aquí iría un diagrama que muestre una población dividida en estratos de colores, y flechas indicando que se toma una muestra aleatoria de cada color).*
...
## Poniendo todo junto: Diseños Muestrales Complejos (Bietápicos) {.smaller background-color="white"}
...
![](img/fiabilidad_validez_diagrama.png){fig-align="center"}
*(Nota: Aquí iría un diagrama visual de cuatro dianas como el de la Figura 5.2 de Babbie).*
...
![](img/error_total_triangulo.png){fig-align="center"}
*(Nota: Aquí iría un diagrama simple como el de Ritchey (Figura 2-2), que muestra el error total como la hipotenusa de un triángulo con el error de muestreo y el error de medición como sus catetos).*
...
**En grupos, discutan:**
1.  **Población y Marco Muestral:** ¿Cuál es la población objetivo? ¿Qué marco muestral usarían (ej. lista de matriculados)? ¿Qué sesgos de cobertura podría tener ese marco (ej. estudiantes con matrícula suspendida)?  
2.  **Fiabilidad:** Si usan una pregunta como "¿Qué tan feliz es usted en una escala de 1 a 7?", ¿qué problemas de fiabilidad podrían surgir (ej. la interpretación de "feliz" puede cambiar según el día o el estado de ánimo)?  
3.  **Validez:** ¿La pregunta "¿Qué tan feliz es usted?" es una medida válida de "bienestar subjetivo"? ¿Qué otras dimensiones importantes del bienestar podría estar omitiendo (ej. estrés, sentido de propósito, salud física)?  
