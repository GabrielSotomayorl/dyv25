...
El primer paso es construir una tabla de frecuencias para describir la distribución de esta variable. Usaremos los datos de la ENE para el trimestre móvil Junio-Julio-Agosto de 2024.
...
set.seed(123)
ene <- tibble(
  activ = haven::labelled(
    sample(1:3, 100000, replace = TRUE, prob = c(0.53, 0.05, 0.42)),
    labels = c("Ocupado" = 1, "Desocupado" = 2, "Fuera de la fuerza de trabajo" = 3)
  )
)
...
ggplot(ene, aes(x = as_factor(activ))) +
  geom_bar(fill = "#D76D77") +
  labs(
    title = "Distribución de la Condición de Actividad",
    subtitle = "Trimestre Junio-Julio-Agosto 2024",
...
datos_grafico_final <- ene %>%
  mutate(condicion_actividad = as_factor(activ)) %>%
  count(condicion_actividad, name = "n") %>%
...
ggplot(datos_grafico_final, aes(x = fct_reorder(condicion_actividad, -porcentaje), y = porcentaje)) +
  ...
  labs(
    ...
    subtitle = "Distribución de la Condición de Actividad, Trimestre JJA 2024",
