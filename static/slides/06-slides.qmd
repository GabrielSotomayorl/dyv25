...
## `filter()`: Trabajando con Filas {.smaller background-color="white"}

`filter()` nos permite quedarnos solo con las observaciones que cumplen una o más condiciones lógicas.

**Sintaxis:** `datos %>% filter(condicion_logica)`

-   **Operadores de Comparación:** `==` (igual a), `!=` (no igual a), `>` (mayor que), `<` (menor que).
-   **Operadores Lógicos:** `&` (y), `|` (o), `!` (no).
-   **Operador de Pertenencia:** `%in%`.

```{r, eval=FALSE, echo=TRUE}
# Filtrar para quedarse solo con personas de la Región Metropolitana (código 13)
casen_rm <- casen %>% 
  filter(region == 13)

# Filtrar por mujeres (código 2) mayores de 60 años
mujeres_mayores <- casen %>% 
  filter(sexo == 2 & edad > 60)

# Filtrar por personas de las regiones de Valparaíso (5) o Biobío (8)
casen_v_viii <- casen %>% 
...
## La Combinación más Poderosa {.smaller background-color="white"}
